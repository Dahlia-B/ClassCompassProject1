...
namespace ClassCompassApi_Fresh.Infrastructure.Sockets
{
    public class TcpSocketServer : BackgroundService
    {
        ...
        private async Task BroadcastMessageAsync(SocketMessage message, string excludeClientId = "")
        {
        ...
        private ConnectedClient GetClientByUserId(string userId)
        {
            lock (_clientsLock)
            {
                return _clients.Find(c => c.UserId == userId) ?? new ConnectedClient();
            }
        }
